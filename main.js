...
function checked_task_to_down(id_item, list) {
    let task_and_button = ''
    for (task_index in list) {
        if (list[task_index].includes(id_item)) {
            task_and_button = list[task_index]
            list.splice(task_index, 1)
            list.push(task_and_button)
            return list
        }
    }
}
...
function checked_task(item_id, btn_id) {
    ...
    let task_list = checked_task_to_down(item_id, todas_as_tarefas);
    document.getElementById("area-tarefa").innerHTML = ''
